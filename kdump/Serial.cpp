...
void QueryKey(HKEY hKey,VEC_STR& vec) 
{ 
...
void CSerial::GetComList(VEC_STR& vec)
{
	vec.clear();
	HKEY hRoot;;
	if( RegOpenKeyEx(HKEY_LOCAL_MACHINE,_T("HARDWARE\\DEVICEMAP\\SERIALCOMM"),NULL,KEY_READ,&hRoot) ==S_OK)
	{
		QueryKey(hRoot,vec);
		RegCloseKey(hRoot);
	}
...
LONG CSerial::Open (LPCTSTR lpszDevice, DWORD dwInQueue, DWORD dwOutQueue, bool fOverlapped)
{
	...
	TCHAR al_Com[_MAX_FNAME];
	if(lpszDevice[0]!=_T('\\'))
	{
		_sntprintf_s(al_Com,_MAX_FNAME,_T("\\\\.\\%s"),lpszDevice);
	}
	else
	{
		_tcsncpy_s(al_Com,lpszDevice,_MAX_FNAME);
	}

	// Open the device
	m_hFile = ::CreateFile(al_Com,
						   GENERIC_READ|GENERIC_WRITE,
						   0,
						   0,
						   OPEN_EXISTING,
	...
	{
		commConfig.dcb.BaudRate = DWORD(m_defBaudrate);
		commConfig.dcb.ByteSize = BYTE(m_defDataBits);
		commConfig.dcb.Parity   = BYTE(m_defParity);
		commConfig.dcb.StopBits = BYTE(m_defStopBits);

		// Determine if parity is used
		commConfig.dcb.fParity  = (m_defParity != EParNone);
