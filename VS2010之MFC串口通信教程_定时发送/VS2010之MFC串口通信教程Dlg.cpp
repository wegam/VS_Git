...
//************************************************************窗口打印--开始
		...
		pMyClass->SetDlgItemText(IDC_EDIT_Dcont,m_EDIT_Dcont);//不使用UpdateData(FALSE);的原因是该函数会刷新整个对话框的数据，而SetDlgItemText()只更新接收编辑框的数据。
		...
		pMyClass->GetDlgItemText(IDC_EDIT_Rxcont, m_EDIT_Rxcont);

		m_Rxcont = _ttoi(m_EDIT_Rxcont);
		m_Rxcont += len;
		m_EDIT_Rxcont.Format(_T("%d"), m_Rxcont);

		//pMyClass->GetDlgItem(IDC_EDIT_Rxcont))->SetLimitText(m_Rxcont);
		pMyClass->SetDlgItemText(IDC_EDIT_Rxcont, m_EDIT_Rxcont);//不使用UpdateData(FALSE);的原因是该函数会刷新整个对话框的数据，而SetDlgItemText()只更新接收编辑框的数据。
...
BOOL CVS2010之MFC串口通信教程Dlg::OnInitDialog()			//初始化对话框程序
{
	...
	SetDlgItemText(IDC_EDIT_Rev, m_EDIT_Dcont);//不使用UpdateData(FALSE);的原因是该函数会刷新整个对话框的数据，而SetDlgItemText()只更新接收编辑框的数据。

	//初始化已接收字节数计数
	//CEdit* pEDIT_Rxcont = (CEdit*)GetDlgItem(IDC_EDIT_Rxcont);
	//long  m_Rxcont = 0;
	//CString m_EDIT_Rxcont = _T("");
	////((CEdit*)GetDlgItem(IDC_EDIT_Rxcont))->SetLimitText(m_Rxcont);
	////m_Rxcont = pEDIT_Rxcont->GetLimitText();
	//((CEdit*)GetDlgItem(IDC_EDIT_Rxcont))->SetLimitText(m_Rxcont);
...
void CVS2010之MFC串口通信教程Dlg::OnBnClickedButtonSend()		//发送数据
{
	...
	//SetTimer(1, 1, NULL);		//定时器
	/*
	//滚动条处于最下方，如下的代码将实现这一功能
...
void CVS2010之MFC串口通信教程Dlg::OnBnClickedButtonClean()		//清除数据
{
	...
	//行计数器
	CEdit*pedit = (CEdit*)GetDlgItem(IDC_EDIT_Dcont);
	unsigned long   nline = pedit->GetLimitText();
	nline = 0;
	m_EDIT_Dcont.Format(_T("%d"), nline);
	((CEdit*)GetDlgItem(IDC_EDIT_Rev))->SetLimitText(nline);
	...
	//unsigned long  m_Rxcont = pEDIT_Rxcont->GetLimitText();
	//m_Rxcont += 20;
	//m_EDIT_Rxcont.Format(_T("%ld"), m_Rxcont);
	//((CEdit*)GetDlgItem(IDC_EDIT_Rxcont))->SetLimitText(m_Rxcont);
	//SetDlgItemText(IDC_EDIT_Rxcont, m_EDIT_Rxcont);//不使用UpdateData(FALSE);的原因是该函数会刷新整个对话框的数据，而SetDlgItemText()只更新接收编辑框的数据。

	//显示已接收字节数计数
	//CEdit* pEDIT_Rxcont = (CEdit*)GetDlgItem(IDC_EDIT_Rxcont);
	//long  m_Rxcont = pEDIT_Rxcont->GetLimitText();
	//m_Rxcont = 1;	
	//m_EDIT_Rxcont.Format(_T("%d"), m_Rxcont);
	//SetDlgItemText(IDC_EDIT_Rxcont, m_EDIT_Rxcont);*///不使用UpdateData(FALSE);的原因是该函数会刷新整个对话框的数据，而SetDlgItemText()只更新接收编辑框的数据。
	//((CEdit*)GetDlgItem(IDC_EDIT_Rxcont))->SetLimitText(m_Rxcont);
